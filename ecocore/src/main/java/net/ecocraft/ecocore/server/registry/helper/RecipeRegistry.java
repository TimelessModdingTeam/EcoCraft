    ...
    public RecipeRegistry(Object output, Object... items) {
        ...
        if (items[0] instanceof Character) {
            for (int i = 0; i + 1 < items.length; i += 2) {
                ...
                if (items[i + 1] == null || !(items[i + 1] instanceof ItemStack || items[i + 1] instanceof Item || items[i + 1] instanceof Block ||
                        items[i + 1] instanceof String || items[i + 1] instanceof PseudeItemStack)) {
                    throw new ExceptionInInitializerError();
                }
            ...
            for (int i = 0; i + 1 < items.length; i++) {
                if (items[i] == null || !(items[i] instanceof ItemStack || items[i] instanceof Item || items[i] instanceof Block ||
    ...
    public void registerRecipe() {
        ...
        for (Character c : cs) {
            ...
            if (curObj instanceof ItemStack) {
                ...
                is = ContentObject.getObjReg(name).getItemStack();
            }
        ...
        if (this.output instanceof ItemStack) {
            ...
            outStack = ContentObject.getObjReg(name).getItemStack();
